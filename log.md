...
## 2026-06-28 — Autopilots program: plan v1 + competitive verification

Program-owner work on the cross-platform Autopilots initiative. Strategy artifacts live in the claude.ai Linx project (`/Users/omrilorch/Claude/Projects/Linx Autopilots`); this is the capture + pointer.

- **Built `Autopilots_Program_Plan.md` v1** against five pillars: (1) what/when/why + sequence, (2) ownership map across Noah/DOR/Mo, (3) platform metrics & QA, (4) customer experience, (5) competitor analysis. Program-owner altitude — the seams above the agents, not agent mechanics.
- **Two-initiative boundary locked (Omri correction):** Autopilots = the workforce of identity-security *agents*; **MCP Gateway** = the runtime tool-call enforcement substrate. Separate initiatives. The "block-the-call / tool-call parameter granularity" claim is **Gateway scope, not Autopilots**. Only seam: the Agentic Posture (Shepherd) autopilot uses the Gateway as its action arm.
- **Autopilots competitive verdict (deep-research, adversarial 3-vote, GA-vs-announced):**
  - *Earned autonomy as a product* = **survives, cleanest wedge.** No verified rival graduates autonomy per-tenant by proven precision. Lumos Identity Agent Force (2026-06-15) ships a static escalation model. Medium confidence — Lumos/Saviynt login-gated docs unchecked; confirm before forum.
  - *Custom agents under one governance contract* = **contested, not matched.** Microsoft Security Copilot ships a composable agent-builder naming Identity Governance (Entra Access Review Agent already ships) but governs user-built agents with static RBAC, not earned autonomy; parts prerelease. CrowdStrike AgentWorks = no-code build-your-own but SOC-oriented, GA unconfirmed.
  - Defensible position = the combination no rival holds: earned autonomy x custom-agents-under-the-same-governance x (via Gateway) IDP-agnostic estate-wide enforcement x one identity graph.
- **Gateway-side finding (handed off to the Gateway agent):** AWS Bedrock AgentCore Gateway GA 2026-03-03 — Cedar-policy enforcement on MCP tool *argument values*, matching the parameter-granularity claim, but AWS-ecosystem-only. Decisive open Q: does it reach third-party MCPs (would erode IDP-agnostic edge). Full map in `Gateway_Competitive_Intel_HANDOFF.md`.
- **Fresh moves:** 4 launches in 48h (2026-06-15/16: Lumos, Saviynt IARA, CrowdStrike identity track, Ping cloud extensions); CrowdStrike to acquire SGNL (authorization); Cisco widening post-Astrix; Newcore raised $66M for agent identities.
- **Deck template standard set (Omri directive):** `Linx Webinar - Closing the Identity Risk Gap with Autonomous AI - April 2026.pptx` = canonical theme for all Linx decks (16:9, Arial, brand green #158158).
- **Artifacts produced** (in the Linx Autopilots project): `Autopilots_Program_Plan.md`, `Autopilots_Competitive_Intel.md`/`.txt` (NotebookLM source), `Gateway_Competitive_Intel_HANDOFF.md`, `Gateway_Competitive_Podcast.mp3` + transcript.
- **Open / next:** Pillars 1–4 still v1. Pillar 3 (outcome instrumentation + quality bar) = #1 Q3 risk — `run.dismissed`/`run.resolved` populated for `linx` tenant only, so the quality bar is unmeasurable cross-tenant. Confirm ownership map with Noah/DOR/Mo. Re-verify competitive set the week before any forum (fastest-moving frontier). Forward: stand up an Autopilots repo/project/brain like the Gateway has.
